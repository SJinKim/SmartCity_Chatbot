...
def uploadData(data_folder, vectorestore_name):
    ...
    if os.path.exists(local_index):
        FAISS.load_local(folder_path=local_index, embeddings=embeddings, index_name=vectorestore_name)
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            progress_bar = tqdm(total=total_files, desc=(f"Dokumente aus Ordner < {data_folder} > werden geladen"))
        ...
        def create_text_splitter(documents, max_chunk=2000, min_chunk=400):
            """ Splits documents into smaller chunks using a recursive approach and displays a progress bar.
        ...
        split_docs = create_text_splitter(docs)
        ...
        local_vectorestore = create_faiss_index(embeddings, split_docs)
        index_folder = os.path.join(os.path.dirname(__file__), vectorestore_name)
        local_vectorestore.save_local(folder_path=index_folder, index_name=vectorestore_name)
        print("Index erstellt und gespeichert!")  

        
        """ Loads the saved local Vectorstore index.
        """   
        loaded_data = FAISS.load_local(folder_path=index_folder, embeddings=embeddings, index_name=vectorestore_name)
...
#uploadData(data_folder="Bescheide_docs", vectorestore_name="bescheide_index")
