"""
    This modul creates the prompt template for generating a bescheid.
"""
...
def bescheid_template(sachverhalt: str, pruefungsergebnis) -> PipelinePromptTemplate:
    ...
    2. Den Beispielbescheid und das Muster eines Beispielbescheids analysieren, \
    um Erkenntnisse zu gewinnen.
    Aus einem Sachverhalt wird der folgende Beispiel_Bescheid generiert: {bsl_bescheid}
    3. Unter Berücksichtigung der Sachverhaltsprüfung einen sachlichen und ausführlichen 
    Bescheid verfassen.
    Sachverhaltsprüfung: {sachverhaltspruefung}
    Format: 
    Das zu erstellende Bescheid umfasst fünf Abschnitte:
    1. Einleitung: Briefkopf, Betreff (schlagwortartige Andeutung des Bescheids), \
    Anrede und Überleitungssatz zum Tenor
    ...
    4. Rechtsbehelfsbelehrung: Rechtsbehelf (z.B. Widerspruch, Anfechtungsklage, \
    Berufung, Revision) muss namentlich die Behörde, bei der er einzulegen ist, \
    deren Sitz und die einzuhaltende Frist angeben
    ...
    message = full_template.format(
        ...
        sachverhaltspruefung=pruefungsergebnis,
    )
    return message
